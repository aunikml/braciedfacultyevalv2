...
import { ArrowLeft, Search, FileText, BarChart, Users, BookOpen, ChevronRight, CheckCircle, Clock } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import client from '../api/client';
import { useAuth } from '../context/AuthContext';

const ProgramSupervisorDashboard = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const res = await client.get('/evaluations/program-faculty/');
  ...
  const filteredFaculty = facultyList.filter(f => f.id !== user.id).filter(f =>
    `${f.first_name} ${f.last_name} ${f.email} ${(f.courses || []).join(' ')}`.toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <Container maxWidth="xl">
      <Box sx={{ mb: 4 }}>
        <Typography variant="h4" fontWeight={800}>
          Program <span style={{ color: '#6366f1' }}>Dashboard</span>
        </Typography>
        <Typography variant="body1" color="text.secondary" sx={{ mt: 0.5 }}>
          Faculty members with evaluation instances in your program
        </Typography>
      </Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        <Stack direction="row" spacing={1.5} alignItems="center">
          <Chip
            icon={<Users size={16} />}
            label={`${facultyList.length} Faculty`}
            color="primary"
            ...
            <Grid item xs={12} sm={6} md={4} lg={3} key={f.id}>
              ...
              >
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                  ...
                  <Typography variant="caption" color="text.secondary" noWrap sx={{ width: '100%', mb: 1.5 }}>
                    {f.email}
                  </Typography>
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 10, textAlign: 'center', borderRadius: 4, border: '2px dashed #e2e8f0', bgcolor: 'transparent' }}>
              ...
              <Typography variant="h5" color="text.secondary" fontWeight={700}>
                {searchQuery ? 'No faculty match your search' : 'No faculty members found'}
              </Typography>
              <Typography variant="body1" color="text.secondary" sx={{ mt: 1 }}>
                {searchQuery ? 'Try a different search term' : 'No faculty have evaluation instances in your program yet.'}
              </Typography>
            </Paper>
          </Grid>
        )}
...
export default ProgramSupervisorDashboard;
